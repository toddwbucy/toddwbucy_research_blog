...
| Chunking degrades performance    | TNT (November 2025)        | Hierarchical memory: 1 global + N local modules. 17× faster, better perplexity.         |
...
This is where the community's understanding stops. People know Titans have a memory module that learns during inference. What they don't know — because the arXiv preprint only dropped in December and it takes all seven papers to see this — is that it's not a better Transformer. It's not attention plus memory. It's a claim that the concepts we use to think about neural networks are wrong, and that the apparent diversity of deep learning architectures is an illusion.
...
### The `train()` / `eval` Switch Encodes the Wrong Ontology
...
- A `train()` / `eval` switch that silenced the inner loop during evaluation. The mechanism the papers call "test-time memorization" is disabled at test time.
...
109 tests passed. Every equation is correct. Zero structural compliance with the paradigm the papers described. And a cost profile that makes it clear: yes, you *can* build this with our conventional toolset. However you can also hammer a nail with a pipewrench, but you wouldn't want to build a house that way.
